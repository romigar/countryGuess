...
int main(int argc, char *argv[])
{
    ...
    std::vector<country> newlist;
    std::vector<continent> _continentList;

/*
    read_file(&newlist);
    fillList(newlist,&_continentList);
    std::cout<< "fill finished  "<<  +_continentList.size()<<std::endl;

    for (int i = 0 ; i  < _continentList.size() ; i++) {
        std::cout<< _continentList.at(i).getName()  << "\t"
                 << +_continentList.at(i).getId() << "\t"
                 << +_continentList.at(i).list.size() << "\t"
                 << std::endl;
        _continentList.at(i).display_list_country_name();
    }
*/
